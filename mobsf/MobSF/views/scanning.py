...
def add_to_recent_scan(data):
    ...
    try:
        db_obj = RecentScansDB.objects.filter(MD5=data['md5'])
...
def handle_uploaded_file(content, typ, uuid):
    """Write Uploaded File."""
    md5 = hashlib.md5()
    ...
    anal_dir = os.path.join(settings.UPLD_DIR, uuid + '/')
    ...
    with open(anal_dir + uuid + typ, 'wb+') as destination:
        if bfr:
            content.seek(0, 0)
            while chunk := content.read(8192):
...
class Scanning(object):
    ...
    def scan_apk(self, uuid):
        """Android APK."""
        hash = handle_uploaded_file(self.file, '.apk', uuid)
        data = {
